...
def init_pygame(dimension):
    pygame.init()
    logo = pygame.image.load("onering.jpg")
    pygame.display.set_icon(logo)
    pygame.display.set_caption("Aragorn burning Nazguls")
...
def moving_drone(current_map, path, speed=0.5):
    ...
    drona = pygame.transform.scale(pygame.image.load("aragorn.png"), (Util.mapLength, Util.mapLength))
    ...
    sensor = pygame.transform.scale(pygame.image.load("nazgul.png"), (Util.mapLength, Util.mapLength))
    running = True
    for cell_i in range(len(path)):
        ...
        for cell in path[:cell_i+1]:
            if current_map.surface[cell[0]][cell[1]] == 2:
                i = cell[0]
                j = cell[1]
                neighbors = [[i, j], [i, j], [i, j], [i, j]]
                for _ in range(cell[2]):
                    for direction_index in range(len(Util.v)):
                        new_neighbor = deepcopy(neighbors[direction_index])
                        new_neighbor[0] += Util.v[direction_index][0]
                        new_neighbor[1] += Util.v[direction_index][1]
                        if 0 <= new_neighbor[0] < current_map.n and 0 <= new_neighbor[1] < current_map.m:
                            if current_map.surface[new_neighbor[0]][new_neighbor[1]] != 1:
                                neighbors[direction_index] = new_neighbor
                                screen.blit(sighted_cell, (new_neighbor[1] * Util.mapLength, new_neighbor[0] * Util.mapLength))
